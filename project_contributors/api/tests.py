...
# class AddSkillTestCase(APITestCase):
#     def setUp(self):
#         self.user = User.objects.create_user(username='test_user', password='test_password')

#     def test_add_skill_success(self):
#         # Make a POST request to obtain the token
#         response = self.client.post('/api/token/', {'username': 'test_user', 'password': 'test_password'})

#         token = response.data.get('token')
#         self.assertIsNotNone(token)  # Ensure the token is not None

#         # Add the token to the client's credentials for subsequent requests
#         self.client.credentials(HTTP_AUTHORIZATION=f'Token {token}')

#         # Add a skill using the token
#         response = self.client.post('/api/add_skill/', {'skill_name': 'Python'})
#         self.assertEqual(response.status_code, status.HTTP_201_CREATED)
#         self.assertEqual(response.data['message'], 'Skill added successfully')

#     def test_add_existing_skill(self):
#         # Create a skill
#         skill = ProgrammingSkill.objects.create(name='Python')
...
#         # Add the same skill using the token
#         response = self.client.post('/api/add_skill/', {'skill_name': 'Python'})
#         self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
#         self.assertEqual(response.data['message'], 'Skill "Python" already exists')
...
#         response = self.client.post('/api/remove_skill/', {'skill_name': 'Python'})
#         self.assertEqual(response.status_code, status.HTTP_200_OK)
#         self.assertEqual(self.user.programming_skills.count(), 1)  # Check if the skill is removed

#     def test_remove_nonexistent_skill(self):
#         self.client.force_authenticate(user=self.user)  # Authenticate the user
#         # Attempt to remove a nonexistent skill
#         response = self.client.post('/api/remove_skill/', {'skill_name': 'C++'})
#         self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

#     def test_remove_skill_not_owned(self):
#         self.client.force_authenticate(user=self.user)  # Authenticate the user
#         # Attempt to remove a skill not owned by the user
#         response = self.client.post('/api/remove_skill/', {'skill_name': 'Java'})
#         self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
#         self.assertEqual(self.user.programming_skills.count(), 2)  # Check if the skill is not removed
...
#         self.assertEqual(len(response.data), 2)  # Only the "active" project should be returned
...
#         response = client.post(f'/api/projects/{self.project1.id}/express_interest/')

#         # Check if the request was successful (HTTP 201 status code)
#         self.assertEqual(response.status_code, status.HTTP_201_CREATED)

#         # Check if the response contains the expected message
#         self.assertEqual(response.data['message'], 'User expressed interest in the project successfully')
...
#         self.assertFalse(OpenSourceProject.objects.filter(id=self.project.id).exists())
...
#         self.eoi1 = ExpressionOfInterest.objects.create(user=self.user1, project=self.project)
#         self.eoi2 = ExpressionOfInterest.objects.create(user=self.user2, project=self.project)
...
#         response = self.client.get(f'/api/projects/{self.project.id}/interests/')

#         # Check if the request was successful (HTTP 200 status code)
#         self.assertEqual(response.status_code, status.HTTP_200_OK)

#         # Check if the response data matches the expected serialized data
#         serializer = ExpressionOfInterestSerializer([self.eoi1, self.eoi2], many=True)
...
#         # Make a GET request to project_interests endpoint
#         response = self.client.get(f'/api/project_interests/{self.project.id}/')
...
class AcceptOrRejectInterestTestCase(TestCase):
    def setUp(self):
        # Create users
        self.creator_user = User.objects.create_user(username='creator', password='password')
    ...
    def test_accept_interest(self):
        client = APIClient()
        client.force_authenticate(user=self.creator_user)  # Authenticate creator user
        
        # Make a POST request to accept interest endpoint
        response = client.post(f'/api/projects/{self.project.id}/accept_or_reject_interest/{self.eoi.id}/', {'action': 'accept'})
        
        # Check if the request was successful (HTTP 200 status code)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        
        # Check if the interest was accepted
        self.assertEqual(ExpressionOfInterest.objects.get(id=self.eoi.id).status, 'accepted')
    ...
    def test_accept_already_accepted_interest(self):
        ...
        self.assertEqual(response.data['message'], 'User is already accepted for this project')
